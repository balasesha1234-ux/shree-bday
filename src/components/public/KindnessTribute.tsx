...
export const KindnessTribute: React.FC = () => {
  ...
  return (
    <section id="kindness-tribute" className="w-full max-w-6xl mx-auto px-4 py-20 select-none">
          ...
          return (
            ...
            >
              ...
              <div className="mt-6 pt-3 border-t border-pink-200/50 flex items-center justify-between text-xs text-gray-500 font-caveat text-base">
                ...
                <span className="font-space text-[10px] text-[#FF4D8D] font-bold">TAP TO EXPAND ➔</span>
              </div>
            </motion.div>
          );
        })}
      ...
      {/* Rich Popup Modal for Kindness Card */}
      <AnimatePresence>
        {selectedCard && (
          <motion.div
            initial={{ opacity: 0 }}
          ...
          >
            <motion.div
              ...
              className="relative max-w-2xl w-full bg-white rounded-3xl p-6 sm:p-8 shadow-2xl border-2 border-pink-200 overflow-hidden"
            >
              {/* Close Button */}
              <button
                onClick={() => setSelectedCard(null)}
                className="absolute top-4 right-4 w-9 h-9 rounded-full bg-gray-100 hover:bg-gray-200 flex items-center justify-center text-gray-600 transition-all z-10"
              >
                <X className="w-5 h-5" />
              </button>

              <div className="space-y-4">
                <div className="w-full aspect-[16/9] rounded-2xl overflow-hidden border border-pink-100 shadow-sm">
                  <img
                ...
                <div className="flex items-center gap-2">
                  <span className="px-3 py-1 rounded-full bg-pink-100 text-[#FF4D8D] text-xs font-space font-bold uppercase">
                    {selectedCard.badge}
                  </span>
                  <span className="text-sm font-quicksand text-gray-500 font-semibold">
                    The Kindness Chronicles 🌸
                  </span>
                </div>

                <h3 className="text-2xl sm:text-3xl font-fredoka font-bold text-gray-800">
                  {selectedCard.title}
                </h3>

                <p className="text-base font-caveat font-bold text-[#FF2D78]">
                  "{selectedCard.quote}"
                </p>
                ...
                <div className="pt-4 border-t border-pink-100 flex items-center justify-between">
                  ...
                  <button
                    ...
                    className="flex items-center gap-2 px-5 py-2.5 rounded-full bg-gradient-to-r from-[#FF4D8D] to-[#FF7A59] text-white font-fredoka font-bold text-xs shadow-md hover:scale-105 active:scale-95 transition-all"
                  >
                    <Heart className="w-4 h-4 fill-white" />
                    <span>
                      {pledgedCards.has(selectedCard.id) ? 'Blessing Sent! 💖' : 'Pledge an Act of Kindness 🪷'}
                    </span>
                  </button>
                </div>
              </div>
